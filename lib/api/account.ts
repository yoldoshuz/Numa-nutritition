 ...
 *
 * `X-Store` is the one moment the backend gets to learn which storefront the
 * client signed up on: it is written to `registrationStore` at the first OTP
 * confirmation and never rewritten, so without the header the account reaches
 * the admin panel as a dash forever. The header is advisory — a missing one
 * must never fail a login, which is why it is not validated here.
 */
